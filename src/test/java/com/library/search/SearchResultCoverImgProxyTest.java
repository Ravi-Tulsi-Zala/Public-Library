...
public class SearchResultCoverImgProxyTest {
	...
	public void setUp() throws Exception {
		...
		SearchResultsPopulator srp = new SearchResultsPopulator();
	...
	public void coverImageURLsAreAsExpectedForDifferentResultPageNumberAndDifferentSessions() {
		...
		SearchResultsPopulator.populateSearchResults(notUsedHere, srForPageXSession1, new MockNumOfItemsInResult(nBooks1Page1, nMovies1Page1, nMusic1Page1));
		srcip.loadCoverImages(srForPageXSession1, session1PageNumber1, mockHttpSession1);
		SearchResultsPopulator.populateSearchResults(notUsedHere, srForPageYSession1, new MockNumOfItemsInResult(nBooks1Page4, nMovies1Page4, nMusic1Page4));
		srcip.loadCoverImages(srForPageYSession1, session1PageNumber4, mockHttpSession1);
		SearchResultsPopulator.populateSearchResults(notUsedHere, srForPageXSession2, new MockNumOfItemsInResult(nBooks2Page1, nMovies2Page1, nMusic2Page1));
	...
	private void assertExpectedPaths(MockHttpSession session, SearchResults sr, String pageNumber) {
		for(List<LibraryItem> resultsPerCategory : sr.getSearchResultsPerCategory()) {
			...
			for(LibraryItem item : resultsPerCategory) {
				...
				urlsExpected.add(PATH_TO_DYNAMIC_CONTENT_DIR + "searchResults" + 
						File.separator + session.getId() + File.separator + pageNumber + File.separator + item.getItemID());
			}
